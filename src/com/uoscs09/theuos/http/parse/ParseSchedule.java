...
public class ParseSchedule extends JerichoParse<ScheduleItemWrapper> {
	...
	protected ParseSchedule(String htmlBody, int howTo) {
		super(htmlBody);
	}

	@Override
	protected List<ScheduleItemWrapper> parseHttpBody(Source source)
			throws IOException {
		// schList와 boardList Element들을 가져옴.
		final List<Element> elements = source.getChildElements().get(1)
				.getChildElements();

		FutureTask<ArrayList<ScheduleItem>> scheduleParseTask = new FutureTask<ArrayList<ScheduleItem>>(
		...
		FutureTask<ArrayList<BoardItem>> boardParseTask = new FutureTask<ArrayList<BoardItem>>(
		...
		ArrayList<ScheduleItemWrapper> list = new ArrayList<ScheduleItemWrapper>();
		ArrayList<ScheduleItem> scheduleList = null;
		ArrayList<BoardItem> boardList = null;
	...
	private static class ParseScheduleInner extends OApiParse<ScheduleItem> {
		private static final String[] PATTERN_ARRAY = { "content", "sch_date",
				"year", "month" };
		...
		protected void initItem(ArrayList<String> parsedStringList,
				ArrayList<ScheduleItem> returningList) {
			returningList.add(new ScheduleItem(parsedStringList
					.toArray(new String[4])));
		}

	}

	private static class ParseBoard extends OApiParse<BoardItem> {
		private static final String[] PATTERN_ARRAY = { "seq", "notice_dt",
				"title", "content" };
		...
		@Override
		protected void initItem(ArrayList<String> parsedStringList,
				ArrayList<BoardItem> returningList) {
			returningList.add(new BoardItem(parsedStringList
					.toArray(new String[4])));
		}
